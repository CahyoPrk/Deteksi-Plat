...
if selected =="Pelatihan":   
    st.title("Pelatihan")
    df = pd.read_csv('D:/Kuliah/Semester 7/PAI/Deteksi Plat/data.csv')
    with st.container():
        ...
        with col_2:
            ...
            st.write('')
            epoch = st.selectbox('Masukan Jumlah Epoch', [10, 25, 50, 100, 250])
    ...
    if st.button('Train'):
        ...
        with st.spinner('Sedang Melakukan Training...'):
            model, history, loss_train, loss_test = training(data, output, test_size, learning_rate, batch, epoch)
